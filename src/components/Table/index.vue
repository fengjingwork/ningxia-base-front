<template>
  <div class="table">
    <el-table :data="tableData" stripe style="width: 100%" header-row-class-name="thead">
      ...
      <el-table-column prop="title" label="标题" width="" header-align="center" align="left" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          <router-link :to="{ path: artPath, query: {id: scope.row.id} }" target="_blank" v-if="crumbData[0].channel != 'read'">{{scope.row.title}}</router-link>
          <router-link :to="{ path: artPath, query: {id: scope.row.id, read: '1'} }" target="_blank" v-else>{{scope.row.title}}</router-link>
        </template>
      </el-table-column>
      <el-table-column prop="releaseDateStr" label="发文日期" width="120" header-align="center" align="center"></el-table-column>
      <el-table-column prop="" label="状态" width="180" header-align="center" align="center" v-if="crumbData[0].channel == 'file' || crumbData[0].channel == 'declare'">
        <template slot-scope="scope">
          <span class="read-btn" @click="jumpRead(scope.row)" v-if="scope.row.hasAnalysis == 1 && crumbData[0].channel == 'file'"><img src="@/assets/images/icon-read-yes.png"/></span>
          <span class="read-btn read-no" @click="" v-if="scope.row.hasAnalysis == 0 && crumbData[0].channel == 'file'"><img src="@/assets/images/icon-read-no.png"/></span>
    ...
    <el-table :data="tableData" stripe style="width: 100%" header-row-class-name="thead" v-if="tableHead">
      <el-table-column v-for="(item,index) in tableHead" :prop="item.prop" :label="item.label" :width="item.width" header-align="center" align="item.align" :show-overflow-tooltip="item.tooltip" :key="item.prop"></el-table-column>
      <!--<el-table-column v-for="(item,index) in tableHead" :prop="item.prop" :label="item.label" :width="item.width" header-align="center" align="item.align" :show-overflow-tooltip="item.tooltip">
        <template slot-scope="scope">
